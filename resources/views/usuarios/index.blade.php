...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-gray-400 mt-1"> Administra los accesos y roles del sistema. </p>
    </div>
    <a href="/usuarios/create" class="px-5 py-2 bg-[#25a5be] hover:bg-[#1d8fa5] text-white rounded-lg transition shadow-lg">
        + Nuevo Usuario
    </a>
</div>

<div class="bg-[#121212]/80 p-4 rounded-lg border border-gray-800 shadow-lg mb-6">
    <form id="form-filtros" action="{{ url('/usuarios') }}" method="GET" class="flex flex-col md:flex-row gap-4">
        <div class="flex-1">
            <input 
                type="text" 
                id="input-buscar"
        ...
        <div class="flex gap-2">
            <a href="{{ url('/usuarios') }}" id="btn-limpiar" class="px-4 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm font-semibold rounded-lg border border-gray-700 transition-colors flex items-center {{ request()->filled('buscar') ? '' : 'hidden' }}">
                Limpiar
            </a>
        </div>
    </form>
